from redd_parameters_house456train import params_appliance
...
SAVE_PATH = '../../../trainingdata/train_houses4/'
AGG_MEAN = 522
AGG_STD = 814
def get_arguments():
...
def main():
    ...
    for h in np.array(params_appliance[appliance_name]['train_build']):
        if h in [5,6]:
            continue
        print(h)
        ...
        app_df = pd.read_table(args.data_dir + '/' + 'house_' + str(h) + '/' + 'channel_' +
                                   str(params_appliance[appliance_name]['channels']
                                       [params_appliance[appliance_name]['train_build'].index(h)]) + '.dat',
                                   sep="\s+",
                                   nrows=nrows,
                                   usecols=[0, 1],
